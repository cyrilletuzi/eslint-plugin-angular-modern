...
Lint rules can implement helpers to fix what is reported. But given that most of the rules of this package for which fixes would be relevant are the ones restricting legacy features, this is not a goal of this tool. Angular already provides automated [migrations](https://angular.dev/reference/migrations) from legacy to new features, and each rule related to an existing migration states it in its documentation.
